...
public class ClassificationView extends VBox {
    ...
    private final OptionOptions imgLib = new OptionOptions(
            ...
            List.of("openslide", "bioformats", "others")
    );
    ...
    private final OptionOptions classes = new OptionOptions(
            "Classes", "Names of classes to be labeled, separated by commas (e.g. <Tumor, Eosinophils> (without braces)).\nLeave empty to automatically extract from model."
    );
    private final OptionOptions ROIClasses = new OptionOptions(
            "ROI Classes", "Only ROIs with these names will be used for inference.\nLeave empty to run on the whole image.\n default: [Region*, Other] (without brackets)"
    );
    private final OptionOptions grades = new OptionOptions(
            "Grades", "Classification grades, e.g. [1, 2] (without brackets).\nLeave empty to disable grade mode, which is suitable for binary or multiclass data.\nFor gradual data, it is recommended to set this value."
    );
    ...
    public OptionOptions getGrades() { return grades; }
    ...
    //-----------------------
    public ClassificationView(QuPathGUI qupath, EnvironmentModel envModel) {

        ClassificationModel model = new ClassificationModel();
        ClassificationController controller = new ClassificationController(qupath, envModel, model, this);
        ...
        grades.addToGridPane(optionsGrid, optionRow++);
