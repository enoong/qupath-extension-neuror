...
classNames = [%s]    // names of classes of interest (in this case two classes, excluding the background class)
...
// ------------------------------------------------------------------------------------
def selectedClassNames = [%s]      // [] or [""] for all region
...
def annotations = getAnnotationObjects().findAll { annotation ->
    annotation.getPathClass() != null && selectedClassNames.any { className ->
        annotation.getPathClass().getName() == className
    } && annotation.getROI()
}
...
def currWSIName = GeneralTools.stripExtension(getProjectEntry().getImageName())
